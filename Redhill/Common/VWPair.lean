...
lemma crtShift_not_dvd {k : ℕ} (hk : k ∈ Icc 3 m) :
  ...
  · rcases lt_or_ge i 2 with hi | hi
    ...
    have meq : crtShift v w m ≡ nonDividingShift v w 4 [MOD 4] :=
      crtShift_modEq (by simp [fourAndOddPrimes])
...
lemma odd_add_crtShift : Odd (w + crtShift v w m) := by
  ...
  have meq : crtShift v w m ≡ nonDividingShift v w 4 [MOD 2] :=
    ModEq.of_dvd (show 2 ∣ 4 by decide) (crtShift_modEq (by simp [fourAndOddPrimes]))
